  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {Object.entries(balances).map(([tokenKey, balance]) => {
        const validTokenKey = tokenKey as keyof typeof tokenConfig;
        const config = tokenConfig[validTokenKey];
        ...
        return (
          ...
          >
            ...
            <p className="text-sm opacity-80 mt-2">
              {CONTRACT_ADDRESSES[validTokenKey as TokenSymbol]?.slice(0, 8)}...
            </p>
          </div>
        );
      })}
